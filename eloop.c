...
void eloop_run(void)
{
	...
	while (!eloop.terminate &&
		...
		if (eloop.sock_table.count != fds_count) {
			if (eloop.sock_table.count == 0) {
				free(fds);
				fds = NULL;
			} else {
				struct pollfd *nfds = (struct pollfd *)
					realloc(fds, eloop.sock_table.count *
			...
			fds_count = eloop.sock_table.count;
		}
